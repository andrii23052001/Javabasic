...
public enum Month {
   ...
   MAY("May",31),JUN("June",30),JUL("July",31),
    AVG("August",31),SEP("September",30),OCT("October",31),
   ...
   /* Задайте emun Month, определите месяца, их наименования и количество в них дней.
    ...
    // метод для увеличения количества месяцев

    public Month plusMounth(int quantity){
        int index= ordinal(); // взяли текущий номер месяца
        index = index+quantity; // увеличили количество месяцев
        Month[]months= values(); // values все константы вносим в массив типа Mounth
        int res= index%months.length; //вычислили номер месяца после прибавления квантити
        return  months[res];// возвращаем обьект типа Mounth , index которого равен % на 12
